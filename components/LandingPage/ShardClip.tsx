import styles from '../../styles/ShardClip.module.scss';

type ShardClipProps = {
    children: React.ReactNode;
};

export const ShardClip = ({ children }: ShardClipProps) => (
    <div className={styles.clip}>
        <svg width="0" height="0" viewBox="0 0 1 1">
            <defs>
                <clipPath id="shardClipPath" clipPathUnits="objectBoundingBox">
                    <path d="M 0.436 0.033 L 0.917 0.34 L 0.519 0.985 L 0.024 0.67 Z " />
                    <path d="M 0.996 0.406 L 0.77 0.79 L 0.967 0.908 L 1.207 0.533 L 1.03 0.429 Z" />
                </clipPath>
            </defs>
        </svg>
        {children}
    </div>
);
